...
public class TelaResultadoEdital extends TelaBase {
    ...
    protected void criarComponentes() {
        ...
        // 2. Painel de Botões
        criarPainelDeBotoes();
        painelBotoes.setBounds(0, 600, 880, 50);
        painelPrincipal.add(painelBotoes);


        // 3. Painel de Resultados (Abas)
        criarPainelDeResultados();
        abasDisciplinas.setBounds(20, 60, 845, 530);
        painelPrincipal.add(abasDisciplinas);
    }
    ...
    private void criarPainelDeResultados() {
        ...
        if (!edital.isResultadoCalculado()) {
            JPanel painelAviso = new JPanel(null); // Mudado para null
            JLabel aviso = criarLabel("O resultado deste edital ainda não foi calculado.", Estilos.FONTE_NORMAL);
            aviso.setBounds(0, 200, 845, 30); // Posicionamento manual
            ...
            for (Disciplina disciplina : edital.getDisciplinas()) {
                ...
                JPanel painelDisciplina = new JPanel(null);
                ...
                if (inscricoesDaDisciplina == null || inscricoesDaDisciplina.isEmpty()) {
                    ...
                    aviso.setBounds(0, 200, 845, 30);
                    aviso.setHorizontalAlignment(SwingConstants.CENTER);
                    painelDisciplina.add(aviso);
                } else {
                    JTable tabelaRanque = criarTabelaRanque(inscricoesDaDisciplina, disciplina);
                    JScrollPane scroll = new JScrollPane(tabelaRanque);
                    scroll.setBounds(0, 0, 840, 500);
                    painelDisciplina.add(scroll);
                }
    ...
    private void criarPainelDeBotoes() {
        ...
        if (isCoordenador()) {
            ...
            if (edital.isResultadoCalculado() && !edital.isPeriodoDesistenciaEncerrado()) {
                ...
                JLabel resultadoFinalLabel = criarLabel("Resultado Final", Estilos.FONTE_NORMAL);
